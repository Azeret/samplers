...
class MassFunction:
    ...
    alpha : float or Iterable[float], optional
        Deprecated synonym for ``slopes`` retained for backward compatibility.
    total_mass : float
    ...
    def __init__(
        self,
        slopes: Iterable[float] | float | None = None,
        *,
        total_mass: float,
        M_min: float,
        M_max_theory: float,
        ...
        """Construct a mass function.

        ``slopes`` is the preferred keyword.  ``alpha`` is accepted for
        backward compatibility with older code.
        """

        if slopes is None:
            if alpha is None:
                raise TypeError("MassFunction requires 'slopes' (or 'alpha')")
            slopes = alpha
